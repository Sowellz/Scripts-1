...
function pont(){
    ...
    sy.get(turl, (error, response, data) => {
      ...
      if (result.success == true) {
        subTitle = `签到结果: 重复`
        detail = `鲜豆：${result.result.userInfoResponse.points}，已签到${result.result.sevenDaysRewardResponse.alreadySignInDays}天，${result.result.sevenDaysRewardResponse.tomorrowSingInRewardText}`
      } else {
        subTitle = `签到结果: 未知`
