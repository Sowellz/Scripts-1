 ...
 * quantumultx
 ...
 [Loon]
 http-request ^https?:\/\/e\.189\.cn\/store\/user\/package_detail\.do script-path=https://raw.githubusercontent.com/cronscript/script/master/china_telecom.js
 # MITM = e.189.cn
 */
...
async function cron() {
    ...
    await parseData(detail, balance, info)
}
...
function parseData(detail, balance, info) {
    return new Promise(resolve => {
        if (!info || !detail || !balance) {
            resolve("done")
            return
        }
        if (balance.result != 0) {
        ...
        notify(detail, balanceAvailable, info)
        resolve("done")
    })
}


function notify(data, balance, exdata) {
